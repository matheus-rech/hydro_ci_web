...
const NiftiReader = (() => {
  ...
  async function parse(arrayBuffer) {
    ...
    if (dimX < 1 || dimY < 1 || dimZ < 1) throw new Error(`Invalid dimensions: ${dimX}×${dimY}×${dimZ}`);
    ...
    // srow_x at offset 280, srow_y at 296, srow_z at 312
    const affine = [
    ...
    const affineValid = sformCode > 0 && (
      Math.abs(affine[0][0]) + Math.abs(affine[1][1]) + Math.abs(affine[2][2]) > 0.01
    );
    const finalAffine = affineValid ? affine : [
      [spacingX, 0, 0, 0],
      [0, spacingY, 0, 0],
      [0, 0, spacingZ, 0],
      [0, 0, 0, 1]
    ...
    let data;
    try {
      data = readTypedData(dataBuffer, datatype, totalVoxels, littleEndian);
    } catch(e) {
    ...
    return {
      ...
      data: float32Data,
      header: {
        ndim, datatype, bitpix, voxOffset, sformCode,
        dims: [dimX, dimY, dimZ],
  ...
  function readTypedData(buffer, datatype, count, littleEndian) {
    // For simple cases where buffer is already aligned
    switch (datatype) {
      case NIFTI_TYPE.UINT8: {
        const arr = new Uint8Array(buffer, 0, count);
        return arr;
      }
      case NIFTI_TYPE.INT16: {
        // Need to ensure 2-byte alignment
        const aligned = ensureAligned(buffer, count * 2);
        const arr = new Int16Array(aligned, 0, count);
        // Swap if file endianness differs from system endianness
        if (littleEndian !== isLittleEndianSystem()) {
          swapBytes(arr, 2);
        }
        return arr;
      }
  ...
  function ensureAligned(buffer, byteLength) {
    // Copy to new ArrayBuffer to ensure proper alignment
    const aligned = new ArrayBuffer(byteLength);
    new Uint8Array(aligned).set(new Uint8Array(buffer, 0, Math.min(byteLength, buffer.byteLength)));
    return aligned;
  }
  ...
  function swapBytes(arr, bytesPerElement) {
    // Byte-swap for big-endian data on little-endian system
